...
const DEFAULT_MARKERS = [
  { lat: 37.78, lng: -122.42, label: "Choose" },
  { lat: 51.51, lng: -0.13, label: "Evaluate" },
  { lat: 35.68, lng: 139.69, label: "Decide" },
  { lat: -33.87, lng: 151.21, label: "Recommend" },
  { lat: 1.35, lng: 103.82, label: "Compare" },
  { lat: 55.76, lng: 37.62 },
  { lat: -23.55, lng: -46.63 },
  { lat: 19.43, lng: -99.13 },
  { lat: 28.61, lng: 77.21 },
...
export function Component({
  ...
  const draw = useCallback(() => {
    ...
    // Determine theme colors (Evalora text gradient)
    const isDark = document.documentElement.classList.contains("dark");
    // Light mode: gray-800 (31, 41, 55). Dark mode: gray-200 (229, 231, 235)
    const r = isDark ? 229 : 31;
    const g = isDark ? 231 : 41;
    const b = isDark ? 235 : 55;

    const dotColor = `rgba(${r}, ${g}, ${b}, ALPHA)`;
    const arcColor = `rgba(${r}, ${g}, ${b}, 0.5)`;
    const markerColor = `rgba(${r}, ${g}, ${b}, 1)`;
    ...
    for (const marker of markers) {
      ...
      const isNamed = !!marker.label;
      const currentMarkerColor = isNamed ? "rgba(239, 68, 68, 1)" : markerColor;

      // Pulse ring
      const pulse = Math.sin(time * 2 + marker.lat) * 0.5 + 0.5;
      ctx.beginPath();
      ctx.arc(sx, sy, 4 + pulse * 4, 0, Math.PI * 2);
      ctx.strokeStyle = currentMarkerColor.replace("1)", `${0.2 + pulse * 0.15})`);
      ctx.lineWidth = 1;
      ctx.stroke();
      ...
      if (marker.label) {
        ...
        ctx.fillStyle = markerColor.replace("1)", "0.8)");
